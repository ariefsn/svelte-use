...
<div class="demo-wrap">
  ...
  <div class="actions">
    ...
    {:else}
      <button onclick={() => { connected = false; close(); }}>Disconnect</button>
    {/if}
  </div>
...
<style>
  ...
  .badge-connecting { background: #1e1e1e; color: #facc15; }
  .badge-open       { background: #142a14; color: #86efac; }
  .badge-closed     { background: #1e1e1e; color: #555; }
  ...
  .log-entry.sent { color: #a78bfa; background: #1a1630; }
  .log-entry.recv { color: #86efac; background: #142a14; }
</style>
